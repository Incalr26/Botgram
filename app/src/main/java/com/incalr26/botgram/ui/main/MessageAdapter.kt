...
class MessageAdapter(
    ...
    private val onFileClick: ((MessageEntity, Int) -> Unit)? = null,
    ...
    class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        val adminTag: TextView = view.findViewById(R.id.adminTag)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (rawObj.has("new_chat_members") || rawObj.has("left_chat_member") || rawObj.has("pinned_message") || rawObj.has("new_chat_title")) {
            holder.mainMessageContainer.visibility = View.GONE; holder.systemMessageText.visibility = View.VISIBLE
            ...
            var sysTxt = "系统消息"; val doer = rawObj.optJSONObject("from")?.optString("first_name", "未知") ?: "未知"
            ...
            } else if (rawObj.has("left_chat_member")) { val left = rawObj.getJSONObject("left_chat_member").optString("first_name", ""); sysTxt = if (doer == left) "$doer 离开了聊天" else "$doer 移除了 $left"
            } else if (rawObj.has("pinned_message")) { sysTxt = "$doer 置顶了消息" } else if (rawObj.has("new_chat_title")) { sysTxt = "$doer 修改了名称" }
        ...
        if (isOutgoing) {
            holder.avatar.visibility = View.GONE; holder.avatarFallback.visibility = View.GONE
            ...
            holder.avatarFallback.visibility = View.VISIBLE; holder.avatar.visibility = View.GONE
            ...
            holder.avatar.setOnClickListener(clickAvatar); holder.avatar.setOnLongClickListener(longClickAvatar)
            holder.avatarFallback.setOnClickListener(clickAvatar); holder.avatarFallback.setOnLongClickListener(longClickAvatar)
        }

        val fromObj = rawObj.optJSONObject("from")
        val combinedName = listOf(fromObj?.optString("first_name", "") ?: "", fromObj?.optString("last_name", "") ?: "").filter { it.isNotEmpty() }.joinToString(" ").takeIf { it.isNotEmpty() } ?: message.senderName ?: "未知"
        holder.senderName.text = combinedName

        val role = message.senderRole ?: ""
        if (role == "creator" || role == "administrator" || role == "owner") {
            holder.adminTag.visibility = View.VISIBLE
            holder.adminTag.text = if (role == "creator" || role == "owner") "所有者" else "管理员"
        } else holder.adminTag.visibility = View.GONE
        ...
        if (rawObj.has("forward_origin") || rawObj.has("forward_from") || rawObj.has("forward_from_chat")) {
            ...
            holder.forwardInfo.text = "转发自 $fName"; holder.forwardInfo.visibility = View.VISIBLE
        } else holder.forwardInfo.visibility = View.GONE

        if (!message.replyToJson.isNullOrEmpty()) {
            try { val replyMsg = JSONObject(message.replyToJson); holder.replyName.text = replyMsg.optJSONObject("from")?.optString("first_name", "未知"); holder.replyPreview.text = replyMsg.optString("text", "[媒体消息]"); holder.replyContainer.visibility = View.VISIBLE } catch (e: Exception) { holder.replyContainer.visibility = View.GONE }
        } else holder.replyContainer.visibility = View.GONE

        holder.mediaContainer.visibility = View.GONE; holder.fileContainer.visibility = View.GONE
        ...
        if (rawObj.has("photo") || rawObj.has("sticker") || rawObj.has("video")) {
            ...
            if (rawObj.has("photo")) {
                ...
                val stickerEmoji = stickerObj.optString("emoji", "❓"); mediaLabel = "[贴纸 $stickerEmoji] "
                ...
                fileId = rawObj.getJSONObject("video").optJSONObject("thumbnail")?.optString("file_id") ?: rawObj.getJSONObject("video").getString("file_id")
                fileSize = rawObj.getJSONObject("video").optLong("file_size", 0L); autoCache = false
            }
            ...
            if (savedPath != null && File(savedPath).exists()) {
                ...
                holder.mediaOverlaySize.text = formatSize(fileSize); holder.mediaOverlay.visibility = View.VISIBLE
                if (autoCache) { holder.mediaOverlay.visibility = View.GONE; onFileClick?.invoke(message, position) }
            }
            holder.mediaOverlay.setOnClickListener { onFileClick?.invoke(message, position) }
            holder.mediaImage.setOnClickListener { val p = fileCachePrefs.getString(currentMsgId.toString(), null); if (p != null && File(p).exists()) onPhotoPreview?.invoke(p) else onFileClick?.invoke(message, position) }
            ...
            holder.fileNameText.text = doc.optString("file_name", if (isAudio) "音频消息" else "未知文件"); holder.fileSizeText.text = formatSize(doc.optLong("file_size", 0L))
            holder.fileContainer.setOnClickListener { onFileClick?.invoke(message, position) }
        }
        ...
        if (actualText.isEmpty() && !message.isDeleted) holder.messageText.visibility = View.GONE
            ...
            for (span in urlSpans) {
                val start = builder.getSpanStart(span); val end = builder.getSpanEnd(span); val url = span.url
                builder.removeSpan(span)
                builder.setSpan(object : ClickableSpan() { override fun onClick(widget: View) { onLinkClick?.invoke(url) } }, start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
            }
            ...
            holder.messageText.text = builder; holder.messageText.movementMethod = LinkMovementMethod.getInstance(); holder.messageText.setTextIsSelectable(true)
        }
            ...
            try {
                ...
                if (arr.length() > 0) {
                    ...
                    for (i in 0 until arr.length()) {
                        ...
                        val count = if (r.has("total_count")) r.getInt("total_count") else r.optInt("count", 1); val isChosen = r.optBoolean("chosen", false)
                        val tv = TextView(ctx).apply { text = "$emojiStr $count"; textSize = 12f
        ...
        val cal = Calendar.getInstance(); val currentYear = cal.get(Calendar.YEAR)
        cal.time = Date(message.date * 1000); val msgYear = cal.get(Calendar.YEAR)
        val timeStr = SimpleDateFormat("HH:mm:ss", Locale.getDefault()).format(cal.time)
        val dateStr = if (currentYear == msgYear) SimpleDateFormat("M月d日", Locale.getDefault()).format(cal.time) else SimpleDateFormat("yy年M月d日", Locale.getDefault()).format(cal.time)
        val editStr = if (message.isEdited) " [已编辑]" else ""
        holder.messageInfo.text = "$mediaLabel ID:${message.messageId}  $dateStr $timeStr$editStr"
        ...
        if (!isOutgoing && prefs.getBoolean("use_real_avatar", true) && message.senderUserId != null) {
            holder.loadJob = scope.launch {
                ...
                if (!url.isNullOrEmpty()) { Coil.imageLoader(ctx).enqueue(ImageRequest.Builder(ctx).data(url).crossfade(true).transformations(CircleCropTransformation()).listener(onSuccess = { _, res -> if (holder.boundMessageId == currentMsgId) { holder.avatar.setImageDrawable(res.drawable); holder.avatarFallback.visibility = View.GONE; holder.avatar.visibility = View.VISIBLE } }).build()) }
            }
        }
