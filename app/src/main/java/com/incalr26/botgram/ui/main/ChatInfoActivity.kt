...
import com.incalr26.botgram.databinding.ActivityChatInfoBinding
...
class ChatInfoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.collapsingToolbar.setContentScrimColor(primaryColor)
        ...
        if (avatarPath.isNotEmpty()) {
            ...
            binding.chatInfoAvatar.load(File(avatarPath)) { crossfade(true) }
            binding.miniToolbarAvatar.load(File(avatarPath)) { crossfade(true); transformations(CircleCropTransformation()) }
        }

        // 小头像点击：展开面板并平滑滚动到顶部
        binding.miniToolbarContent.setOnClickListener {
            binding.appBarLayout.setExpanded(true, true)
            binding.nestedScrollView.smoothScrollTo(0, 0)
        }

        // 大头像点击：全屏预览
        binding.chatInfoAvatar.setOnClickListener {
            showAvatarPreview()
        }

        // 下拉越界触发全屏预览
        var startY = 0f
        binding.appBarLayout.setOnTouchListener { _, event ->
            when (event.action) {
                MotionEvent.ACTION_DOWN -> { startY = event.y }
                MotionEvent.ACTION_MOVE -> {
                    if (binding.appBarLayout.bottom >= binding.appBarLayout.height && (event.y - startY) > 250f) {
                        showAvatarPreview()
                        startY = event.y
                    }
                }
        ...
        binding.appBarLayout.addOnOffsetChangedListener { _, verticalOffset ->
            if (kotlin.math.abs(verticalOffset) >= binding.appBarLayout.totalScrollRange) {
                binding.miniToolbarContent.visibility = View.VISIBLE
            } else {
                binding.miniToolbarContent.visibility = View.GONE
            }
        }
    ...
    private suspend fun loadChatInfo() {
        ...
        val chat = if (resultJson != null && resultJson.getBoolean("ok")) {
            ...
            if (localChat != null) {
                JSONObject().apply {
                    put("id", localChat.chatId)
                    put("type", localChat.type)
                    put("title", localChat.title ?: "")
                    put("first_name", localChat.firstName ?: "")
                    put("last_name", localChat.lastName ?: "")
                    put("username", localChat.username ?: "")
                }
            } else null
        }

        if (chat != null) {
            withContext(Dispatchers.Main) { renderUI(chat) }
        }
        ...
        if (avatarPath.isEmpty()) {
            ...
            if (!avatarUrl.isNullOrEmpty()) {
                ...
                withContext(Dispatchers.Main) {
                    binding.chatInfoAvatar.load(avatarUrl) { crossfade(true) }
                    binding.miniToolbarAvatar.load(avatarUrl) { crossfade(true); transformations(CircleCropTransformation()) }
                }
            }
        }
    }
    ...
    private fun renderUI(chat: JSONObject) {
        ...
        binding.collapsingToolbar.title = title.trim()

        val container = binding.infoContentContainer
        ...
        basicInfo.add("类型" to when (type) {
            "private" -> "私聊"
            "group" -> "私密群组"
            "supergroup" -> if (username.isNotEmpty()) "公开群组" else "私密群组"
            "channel" -> if (username.isNotEmpty()) "公开频道" else "私密频道"
            else -> type
        })
        ...
        if (type != "private") {
            ...
            if (chat.has("join_by_request")) {
                extraInfo.add("加群邀请链接审批" to if (chat.getBoolean("join_by_request")) "开启（需管理员手动通过）" else "关闭")
            }
    ...
    private fun Context.getColorAttr(attr: Int): Int {
        val tv = TypedValue()
